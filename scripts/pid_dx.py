...
class Pid_Controller_Dx():

	def __init__(self):
		rospy.init_node('pid_controller_dx')

		# Parmetri PID
		self.Kp = rospy.get_param('~Kp',5.5106)
		self.Ki = rospy.get_param('~Ki',8.4366)
		...
		self.encoder = rospy.ServiceProxy('vel_encoder_dx',VelRuota)
		
		# Topic
		rospy.Subscriber('vel_ref', Float32, self.ref_callback) 
		self.pub_motor = rospy.Publisher('motor_cmd_dx', Float32, queue_size=10)
...
if __name__ == '__main__':
	pid_controller = Pid_Controller_Dx()
	pid_controller.spin()
